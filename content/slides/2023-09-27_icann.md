...
summary: Accurate Detection of Spiking Motifs by Learning Heterogeneous Delays of a Spiking Neural Network
...
The core idea of the method follows the use of polychronous groups as defined by Izhikevich in 2006. Suppose three presynaptic neurons are connected to two postsynaptic neurons by certains weights and certain delays, which correspond to the time it takes for a spike to travel from one neuron to the next. If we assume these delays are different, then if presynaptic neurons are activated synchronously, then postsynaptic currents do not match in time, such that the membrane potential is not reached. However, if the timing of presynaptic spikes forms a *spiking motif* such that they reach the soma of neuron b_1 at the same time then this neuron will be selectively activated. 
{{< /speaker_note >}}
...
{{< speaker_note >}}
In this work, this principle was framed in a probabilistic setting such that we could provide an optimal scheme for detecting generic spiking motifs which may be superposed at random times. Starting with 10 presynaptic inputs, this model allows to generate a synthtic raster plot as the combination of four different spiking motifs.
These motifs are defined by a positive (red) or negative (blue) contribution to the spiking probability which are represented here.
...
The beauty of this is that we can recover in the presynaptic raster plot the contribution of each spiking motif to the original raster plot.
{{< /speaker_note >}}
...
{{< speaker_note >}}
As a conclusion, this heterogenous delay spiking neural network provides an efficient neural computation. It has some limitations that we detail in the paper, notably that it works on discrete time, yet we hope to deliver soon an unsupervised learning method using this computational brick which could be used to build novel SNNs but also to analyse neurobiological data.
...
<img src="https://laurentperrinet.github.io/talk/2023-09-27-icann/qrcode.png" alt="qrcode" width="50%"/>
