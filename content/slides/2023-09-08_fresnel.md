...
{{< speaker_note >}}
First of all, the general aim of imaging is to represent a light signal, i.e. a luminous intensity, a color, distributed over the visual field, giving us a vivid impression of the visual scene before our eyes. 
{{< /speaker_note >}}
...
{{< speaker_note >}}
This technique is inspired by the research carried out by Étienne-Jules Marey (https://en.wikipedia.org/wiki/Étienne-Jules_Marey), who gave his name to the ISM, under the term chronophotography, which notably enabled later Muybridge to demonstrate the mechanism of a horse's gallop. In particular, Marey literally used a camera mounted on a gun-like structure to shoot a visual scene. 
{{< /speaker_note >}}
...
{{< speaker_note >}}The use of such dynamic visualization is crucial in the scientific field, whether in biology or physics, as it enables us to quantify the characteristics of the experiment being carried out - I'm thinking, for example, of quantifying the movements and number of bacteria in a biological assay.
{{< /speaker_note >}}
...
{{< speaker_note >}}
In the laboratory, we use it in particular to quantify eye movements when a stimulus is presented to an observer.
{{< /speaker_note >}}
...
To better understand the mechanism behind this technology, let's imagine that we represent a single pixel in the space of the visual field. Here, I've taken a video from an episode from the Monty Python Flying Circus TV series. In this way, we can represent the evolution of the log intensity of the light signal as a function of time.
...
{{< speaker_note >}}
First of all, the signal will evolve over time, and we can see here that it may cross a threshold. An event will then be produced by this pixel. Here, the event is of negative polarity, as it corresponds to an decrement.
{{< /speaker_note >}}
...
{{< speaker_note >}}
Then, the signal will continue its course in time and cross a threshold again, possibly once more, at which point a new event will be produced. Here, we're seeing decrements, or negative polarizations.
...
{{< speaker_note >}}
This ability to adapt to changing light conditions can be illustrated by going back to our analog signal and its event representation, and imagining that we have a halving of the signal (a cloud?) after a certain period of time.
...
{{< speaker_note >}}
This is represented here by a sharp decrement in log intensity space. This is clearly indicated by events of negative polarity, but we can see that since this is a camera that uses log intensity, dividing the light signal produces the same signal course over time, and therefore events that are identical.  Event-driven cameras are therefore particularly well-suited to dynamic signals, where the lighting context can change drastically. A typical example would be an autonomous car driving in daylight, entering and leaving a tunnel, involving changes in brightness by a factor of several thousand.
...
These observations have led us to gather neurobiological evidence around the existence of a neural representation that would use the relative time of spikes as a means of representing information. In particular, it is possible to use the conduction delays that exist in the transmission of spikes from one neuron to another. It may seem paradoxical, but these delays are not simply a constraint, but can help to improve our ability to represent information.
...
When using heterogeneous delays, the situation is different, as the information will take a differential time to arrive or not at the neuron's cell body. Note that if we include a particular pattern, which we have here highlighted by green action potentials, then these converge at the same instant thanks to the delay. We will therefore have a detection in the neuron in the form of a new impulse.
