...
$$
(f \ast g)[x, y] = \sum_{i=-K}^{K} \sum_{j=-K}^{K} f[i-x, j-y] \cdot g[i, j]
$$
...
* **Correlation**  of an image (two-dimensional) with a kernel $g$ of radius $K\times K$:
...
* Correlation of an image on several output channels (note [the order of the indices](https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html)):
...
* Correlation of a multi-channel (e.g. RGB) image on multiple output channels (note [the order of the indices](https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html)):
...
{{< figure src="https://stanford.edu/~shervine/teaching/cs-230/illustrations/architecture-cnn-fr.jpeg" title="[[Amidi & Amidi](https://stanford.edu/~shervine/teaching/cs-230/cheatsheet-convolutional-neural-networks)]" width="99%" >}}
